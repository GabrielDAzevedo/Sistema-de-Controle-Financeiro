...
// ==========================================
// Utilitários (datas e validações)
// ==========================================
const Util = {
    // Transações usam 'YYYY-MM-DD'
    toISODate(d) {
    ...
    // Histórico de caixinhas atualmente pode estar em pt-BR; padronizamos para 'YYYY-MM-DD'
    // Aceita 'DD/MM/YYYY' ou 'DD/MM/YY' (quando gerado por pt-BR)
    parsePtBRDateToISO(s) {
...
const Dashboard = {
    ...
    calcularFechamento() {
        ...
        const diaStrBk = `${year}-${month}-${day}`; // ISO para comparar com histórico normalizado
        ...
        transacoes.forEach(t => {
            ...
            if(match) {
                // Aporte não deve ser somado como Receita (já vai para Caixinha),
                // mas entra no balanço via 'ap'
        ...
        Object.values(bancos).forEach(banco => {
            banco.historico.forEach(h => {
                if (h.tipo === 'rendimento') {
                    const hISO = Util.parsePtBRDateToISO(h.data);
                    if (!hISO) return;
                    // Para 'dia' e 'mês' comparamos em ISO 'YYYY-MM-DD'
                    if (this.periodoFechamento === 'dia') {
                        if (hISO === diaStrBk) rend += h.valorMovimento;
                    } else {
                        if (hISO.startsWith(mesStrTx)) rend += h.valorMovimento;
                    }
                }
            });
        });
    ...
    init() {
        document.getElementById('bancos-container')?.addEventListener('click', (e) => {
            ...
            if(btnDelHist) {
                ...
                if (reg.tipo === 'rendimento' || reg.tipo === 'aporte') {
                    bancos[banco].saldo = Math.max(0, bancos[banco].saldo - reg.valorMovimento);
                    // Se o histórico removido foi um rendimento, não devemos ajustar CC aqui,
                    // porque o CC só é afetado em 'withdraw' e em transações.
                } else if (reg.tipo === 'retirada') {
                    // Retirada (vai para CC) foi subtração do saldo da caixinha; ao remover,
                    // devolvemos o saldo da caixinha (e mantemos CC como estava).
                    bancos[banco].saldo += reg.valorMovimento;
                    // Obs: o sistema não mantém vínculo automático do CC ao histórico,
                    // então não ajustamos CC para evitar dupla contagem.
                }
        ...
        document.getElementById('lista-emprestimos')?.addEventListener('click', (e) => {
            ...
            if(e.target.classList.contains('parcela-check')) {
                ...
                if (isChecked) {
                    ...
                    // Cancela a parcela: deve remover a transação da parcela (se existir)
                    // e reverter o efeito no CC.
                    ...
                    if (tIndex !== -1) {
                        ...
                        // Fallback: se não achou a transação, pelo menos reverte o CC
                        // (evita deixar CC alto).
